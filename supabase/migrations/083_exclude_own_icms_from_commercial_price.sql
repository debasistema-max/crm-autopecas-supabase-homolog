...
-- Business invariant: own ICMS is shown in the fiscal memory, but it is not
-- an addition to the commercial price. Keep the legacy column fixed at false
-- so imports and administrative edits cannot reintroduce the old behavior.
update public.fiscal_tax_rules
set resale_include_own_icms = false
where resale_include_own_icms;

alter table public.fiscal_tax_rules
  drop constraint if exists fiscal_tax_rules_own_icms_not_in_price;

alter table public.fiscal_tax_rules
  add constraint fiscal_tax_rules_own_icms_not_in_price
  check (resale_include_own_icms = false);
...
begin
  ...
  -- Own ICMS is informational. It must never increase total_taxes or final_price.
  v_taxes:=round(coalesce(v_ipi,0)+coalesce(v_icms_st,0)+coalesce(v_pis,0)
    +coalesce(v_cofins,0)+coalesce(v_fcp,0),6);
...
comment on function public.calculate_product_price(text,text,text,numeric,date,text)
  is 'Motor fiscal interno: ICMS próprio permanece informativo e nunca compõe total_taxes ou final_price.';
