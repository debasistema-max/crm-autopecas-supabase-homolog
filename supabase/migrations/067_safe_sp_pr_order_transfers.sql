...
-- A missing branch snapshot is unknown, not zero. Transfer requests are created
-- only for SP -> SP orders when both branch snapshots are valid and PR has stock.
...
begin
  ...
  loop
    ...
    if not target_imported then
      warnings := warnings || jsonb_build_array(jsonb_build_object(
        'code','ESTOQUE_SP_NAO_IMPORTADO',
    ...
    if exists (
      ...
      set order_item_id = item_row.order_item_id,
          ...
          reason = 'ORDER_SP_SHORTAGE_PR_TRANSFER',
          updated_at = now()
      where r.order_id = target_order_id
        and r.product_code = item_row.product_code
        and r.source_branch_id = v_pr_branch_id
      ...
      insert into public.stock_transfer_requests(
        ...
        transfer_qty,source_available,target_available,'ORDER_SP_SHORTAGE_PR_TRANSFER',actor.id
      );
...
comment on function public.create_order_transfer_requests(uuid)
  is 'Em pedido SP-SP, cria solicitação PR-SP somente para falta confirmada em SP e saldo importado disponível em PR; ausência de snapshot gera aviso, nunca zero presumido.';
