...
begin
  ...
  with eligible as (
    ...
    select e.*,
      ...
      similarity(e.search_document,v_search_value) as fuzzy_score
    from eligible e
    cross join lateral (
      select
    ...
    where v_search_value=''
       or score.matched_terms>0
       or similarity(e.search_document,v_search_value)>=0.18
  ), catalog as (
    select
      p.*,
      case
  ...
  order by c.exact_code desc,
    case when cardinality(v_tokens)=0 then 0
      else c.matched_terms::numeric/cardinality(v_tokens) end desc,
    c.phrase_match desc,c.field_score desc,c.fuzzy_score desc,c.codigo
...
comment on function public.b2b_search_catalog(text,boolean,integer) is
  'Catálogo B2B por rota; separa termos, prioriza cobertura completa e inclui resultados relacionados.';
