...
begin
  ...
  insert into public.fiscal_tax_rules(
    ...
    base_reduction_rate,freight_rate,insurance_rate,other_expenses_rate,effective_from,effective_to,active,notes,source,created_by,updated_by
  ) values(
  ...
  on conflict(id) do update set ncm=excluded.ncm,uf_origem=excluded.uf_origem,uf_destino=excluded.uf_destino,
    ...
    other_expenses_rate=excluded.other_expenses_rate,effective_from=excluded.effective_from,effective_to=excluded.effective_to,
...
comment on function public.save_fiscal_tax_rule(jsonb) is 'Administra regra fiscal incluindo has_st configurável, despesas e auditoria antes/depois.';
