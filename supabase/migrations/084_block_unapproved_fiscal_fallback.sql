...
-- The internal engine remains callable for homologation and comparison, but
-- cannot replace an absent/non-approved workbook route price in commercial use.
...
begin
  ...
  if v_route_price.product_code is not null
    ...
    v_calc:=jsonb_build_object(
      ...
      'own_icms_included_in_total',false,
    ...
    v_calc:=jsonb_build_object(
      ...
      'ncm',public.normalize_ncm(v_product.ncm),
      'cest',v_product.cest,
      'base_price',case when v_price_imported then v_price.sale_price else null end,
      'total_taxes',null,
      'total_expenses',null,
      'final_price',null,
      'status','PRECO_FISCAL_INDISPONIVEL',
      'warnings',jsonb_build_array(v_route_warning,'FALLBACK_FISCAL_NAO_HOMOLOGADO'),
      'customer_type','REVENDA',
      'requested_customer_type',v_customer_type,
      'calculation_profile','FALLBACK_BLOCKED',
      'calculation_method','NOT_EXECUTED',
...
comment on function public.get_product_commercial_price(text,text,text,date,text)
  is 'Usa somente preço fiscal aprovado do Excel; o motor interno permanece bloqueado para uso comercial até homologação.';

create or replace function public.guard_unapproved_commercial_fiscal_price()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.fiscal_status = 'PRECO_FISCAL_INDISPONIVEL'
     or coalesce(new.fiscal_details->>'price_source','') = 'FISCAL_FALLBACK_BLOCKED' then
    raise exception 'PRECO_FISCAL_INDISPONIVEL: resultado do Excel ausente ou não aprovado';
  end if;
  return new;
end;
$$;

drop trigger if exists guard_unapproved_fiscal_price_order_item on public.order_items;
create trigger guard_unapproved_fiscal_price_order_item
before insert or update of fiscal_status,fiscal_details on public.order_items
for each row execute function public.guard_unapproved_commercial_fiscal_price();

drop trigger if exists guard_unapproved_fiscal_price_quotation_item on public.quotation_items;
create trigger guard_unapproved_fiscal_price_quotation_item
before insert or update of fiscal_status,fiscal_details on public.quotation_items
for each row execute function public.guard_unapproved_commercial_fiscal_price();

comment on function public.guard_unapproved_commercial_fiscal_price()
  is 'Impede pedido ou cotação de persistir preço legado quando o preço fiscal aprovado do Excel está indisponível.';
